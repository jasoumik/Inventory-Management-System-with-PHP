...
if(isset($_GET["pdf"]) && isset($_GET["order_id"])){
    ...
    $pdf = new Pdf();
    $output='';
    $qry="SELECT * FROM inventory_order WHERE invent_ordr_id=:invnt_ordr_id LIMIT 1
    ";
    $stm=$connect->prepare($qry);
    $stm->execute(
        array(
            ':invnt_ordr_id' =>$_GET["order_id"] 
        )
    );
    $result = $stm->fetchAll();
    foreach($result as $row)
    {
       ...
       <table width="100%" cellpadding="5">
        <tr>
         <td width="65%">
          ...
          Name : '.$row["invent_ordr_name"].'<br /> 
          Billing Address : '.$row["invent_ordr_adrs"].'<br />
         </td>
         <td width="35%">
          Reverse Charge<br />
          Invoice No. : '.$row["invent_ordr_id"].'<br />
          Invoice Date : '.$row["invent_ordr_date"].'<br />
         </td>
        </tr>
       </table>
       <br />
     ...
     {
      $count = $count + 1;
      $product_data = fetch_pdt_dtls($sub_row['product_id'], $connect);
    ...
    $file_name = 'Order-'.$row["invent_ordr_id"].'.pdf';
    $pdf->loadHtml($output);
    $pdf->render();
